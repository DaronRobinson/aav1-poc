...
const DefaultGetIdentityFullName = (user: UserType) => `${user.last_name} ${user.first_name}`;
...
export const directusAuthProvider = (
  ...
  const authProvider = {
    ...
    logout: async () => {
      const auth = storage.getItem("auth") ? JSON.parse(storage.getItem("auth")) : undefined;
      if (!auth) {
        return;
      }
    ...
    getPermissions: async () => {
      ...
      const { role } = jwt_decode<JwtPayload & { role: string }>(access_token);
    ...
    getCurrentUser: async () => {
      ...
      if (storage.getItem("user") !== null) {
        console.log("Local storage user", JSON.parse(storage.getItem("user")));
